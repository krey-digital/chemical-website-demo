...
export default function AdminCertificationsPage() {
  ...
  const allCertifications = useQuery(api.certifications.listAll, {});
  // Filter client-side since listAll takes no args
  const certifications = activeFilter === undefined
    ? allCertifications
    : allCertifications?.filter((c) => c.isActive === activeFilter);
  const updateCert = useMutation(api.certifications.update);
  const deleteCertMutation = useMutation(api.certifications.deleteCert);
  const [deletingId, setDeletingId] = useState<string | null>(null);

  const handleActivate = async (id: string) => {
    await updateCert({ id: id as any, isActive: true });
  };

  const handleDeactivate = async (id: string) => {
    await updateCert({ id: id as any, isActive: false });
  };
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Are you sure you want to delete this certification?")) {
      setDeletingId(id);
      try {
        await deleteCertMutation({ id: id as any });
      } finally {
        setDeletingId(null);
      }
    }
